...
def inference(model=None, text=None, sid=0, noise_scale=0.333, noise_scale_w=0.5, length_scale=1.0, device="mps", stream=False, output_file=None, duration_blur_sigma=0.3, temperature=1.0):
    # text is a phoneme string or list of phoneme strings
    if isinstance(text, str):
        chunks = [text]
    else:
        chunks = list(text)
    print(f"Processing {len(chunks)} phoneme chunks")
    ...
    if not stream:
        ...
        for i, chunk in enumerate(tqdm(chunks, desc="Generating audio", unit="chunk")):
    ...
    # --- STREAMING GENERATION ---
        ...
        def audio_generator():
            ...
            pbar = tqdm(total=len(chunks), desc="Generating audio", unit="chunk")
