...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        for entry in self.entries:
            if not os.path.isfile(entry["audio"]):
                continue
            phonemes = entry["phonemes"]
            if not (self.min_text_len <= len(phonemes) <= self.max_text_len):
                continue
            length = os.path.getsize(entry["audio"]) // (2 * self.hop_length)
    ...
    def __getitem__(self, index):
        ...
        text = torch.LongTensor(cleaned_text_to_sequence(entry["phonemes"]))
        mel = torch.load(os.path.join(self.base_dir, entry["mel"]), weights_only=True)
        audio, sampling_rate = utils.load_wav_to_torch(entry["audio"])
        ...
        return (text, mel, audio_norm, sid)
...
def kl_loss(z_p, logs_q, m_p, logs_p, z_mask):
    ...
    kl = torch.clamp(kl, min=0, max=100) * z_mask
    l = torch.sum(kl) / torch.sum(z_mask)
    return l
    ...
    for batch_idx, (
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                kl_warmup = min(1.0, global_step / 1000.0)
                loss_kl = kl_loss(z_p, logs_q, m_p, logs_p, z_mask) * hps.train.c_kl * kl_warmup
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                ...
                utils.remove_old_checkpoints(hps.model_dir, hps.train.keep_checkpoints)
        global_step += 1
